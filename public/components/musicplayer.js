...
function timeUpdate() {
  ...
  //makes sure that the song can actually be played before setting its duration
...
playButton.onclick = function() {
  ...
  //plays next song if the current song has ended
...
function playNext() {
  ...
  music.volume = volumeSlider.value/100; //sets volume of song to the volume of the slider
  ...
  var albumCover = music.parentNode.children[0].src;
  currentCover.src = albumCover;
...
function playPrev() {
  ...
  //changes the displayed song name and artist and album cover
  var songInfo = music.parentNode.children[1].children;
  currentSongName.innerHTML = songInfo[0].innerHTML;
  currentArtist.innerHTML = songInfo[1].innerHTML;
  var albumCover = music.parentNode.children[0].src;
